...
namespace DesktopClient.Modules.Helpers.Series
{
    public static class SeriesHelper
    {
        ...
        public static string Pattern =
                @"^((.+?)[. _-]+)?s(\d+)[. _-]*e(\d+)(([. _-]*e|-)((?!(1080|720)[pi])\d+))*[. _-]*((.+?)((?<![. _-])-([^-]+))?)\.(mkv|avi|mp4|srt)?$"
            ;
        ...
        public static int GetSeasonFromFeliratokInfoEvad(string text)
        {
            ...
            if (regexResult.Count != 0)
                return int.Parse(regexResult[0].Groups[1].Value);
            return -1;
        }
        ...
        public static int GetSeasonFromFeliratokInfoThird(string text)
        {
            ...
            if (regexResult.Count != 0)
                return int.Parse(regexResult[0].Groups[2].Value);
            return -1;
        }
        ...
        public static int GetSeasonNumber(string text)
        {
            var regexResult = regexPattern.Matches(text);
            if (regexResult.Count != 0)
                return int.Parse(regexResult[0].Groups[3].Value);
            return -1;
        }
        ...
        public static int GetEpisodeNumber(string text)
        {
            var regexResult = regexPattern.Matches(text);
            if (regexResult.Count != 0)
                return int.Parse(regexResult[0].Groups[4].Value);
            return -1;
        }
        ...
        public static async Task<InternalSeries> GetShow(string title)
        {
            ...
            var showExist =
                await new WebClientManager().GetShowPost<string>($"http://localhost:5001/series/getseries",
                    requestbody);
            return showExist;
        }
        ...
        public static async Task<int> ImportRequest(string title)
        {
            var requestbody = new InternalImportRequest {Title = title};
            var showExist =
                await new WebClientManager().Post<bool>($"http://localhost:5001/series/import", requestbody);
            return showExist;
        }
        ...
        {
            var isUpdated =
                await new WebClientManager().Post<bool>($"http://localhost:5001/series/updateStartedEpisode/{title}",
                    internalEpisode);
            return isUpdated;
        }
        ...
        public static async Task SavePosition(string showName, int seasonNum, int episodeNum,
            int actualSeenSecondsFromMPC, Times elapsedTimesInMedia)
        {
            ...
            if (percentage <= 98)
                await UpdateStartedSeries(episode, showName);
            else
                await MarkRequest(new InternalMarkRequest
                {
