...
namespace Movie.DataManagement.Parsers.OMDB
{
    public class OmdbParser
    {
        ...
        private Converter.OmdbConverter converter = new OMDB.Converter.OmdbConverter();
        ...
        public async Task<InternalMovie> GetMovieFromOmdb(string title)
        {
            try
            {
                ...
                if (omdbMovie.Title != null)
                {
                    ...
                    int runtime = Int32.Parse(omdbMovie.Runtime.Substring(0, omdbMovie.Runtime.Length - 3));
                    ...
                    return new InternalMovie()
                    {
                        ...
                        Year = Int32.Parse(omdbMovie.Year),
        ...
        {
            var boolean = await new WebClientManager().Get<List<TmdbMovieSimple>>($"{_endpoint}/?t={title}&apikey={_apikey}");
            if (boolean.Count > 0)
            {
